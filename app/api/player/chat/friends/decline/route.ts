...
import { declinePlayerFriendLink } from '@/lib/sql/playerFriendLinks';
import { cleanText } from '@/lib/sql/playerMirrorCommon';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const ROUTE = '/api/player/chat/friends/decline';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await declinePlayerFriendLink({
      actorUid: auth.user.uid,
      otherUid,
    });

    console.info('[PLAYER_CHAT_FRIEND_DECLINE_OK]', {
      route: ROUTE,
      uid: auth.user.uid,
      otherUid,
    ...
    const status =
      /recipient|yourself|required|invalid/i.test(message) ? 400 :
      /inactive|unavailable|not found/i.test(message) ? 404 :
      /forbidden|scope/i.test(message) ? 403 :
      500;
    console.error('[PLAYER_CHAT_FRIEND_DECLINE_ERROR]', message);
