...
import { acceptPlayerFriendLink } from '@/lib/sql/playerFriendLinks';
import { cleanText } from '@/lib/sql/playerMirrorCommon';

export const runtime = 'nodejs';
export const dynamic = 'force-dynamic';

const ROUTE = '/api/player/chat/friends/accept';
...
export async function POST(request: Request) {
  try {
    ...
    const result = await acceptPlayerFriendLink({
      actorUid: auth.user.uid,
      otherUid,
    });

    console.info('[PLAYER_CHAT_FRIEND_ACCEPT_OK]', {
      route: ROUTE,
      uid: auth.user.uid,
      otherUid,
    ...
    const status =
      ...
      500;
    console.error('[PLAYER_CHAT_FRIEND_ACCEPT_ERROR]', message);
