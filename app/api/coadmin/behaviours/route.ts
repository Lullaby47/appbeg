...
function buildStaffBehavioursRows(
  ...
  const playersByStaffId = new Map<string, BehavioursDataDoc[]>();
  ...
  return staffDocs
    ...
    .map((staff) => {
      ...
      const redeemLimitHitPlayers = linkedPlayers.filter((p) => {
        const lockStartedAtMs = toMs((p.redeemWindow24h as AnyDoc | undefined)?.lockStartedAt);
          ...
          riskyPlayers: linkedPlayers
            .filter(
              (p) =>
                ...
                Boolean(p.bonusBlocked) ||
                toMs(p.bonusBlockedUntil) > now ||
                toMs((p.redeemWindow24h as AnyDoc | undefined)?.lockStartedAt) > 0
            )
            .slice(0, 100)
            .map((p) => ({
              playerId: String(p.id),
  ...
  try {
    ...
    if (sqlReadMode) {
      ...
      logCacheSqlRead(ROUTE, {
        ...
        staffCount: data.staffDocs.length,
        playerCount: data.playerDocs.length,
        cashoutCount: data.cashoutDocs.length,
