...
async function fetchSnapshotRowsParallel(
  ...
  for (const row of [...recentResult.rows, ...activeResult.rows]) {
    const firebaseId = cleanText((row as Record<string, unknown>).firebase_id);
    if (firebaseId) {
      merged.set(firebaseId, row as Record<string, unknown>);
    }
  }
...
export async function GET(
  ...
  try {
    ...
    const sqlRequestsStartedAt = Date.now();
    const sqlOutboxStartedAt = Date.now();
    const [snapshotPack, outboxPack] = await Promise.all([
      fetchSnapshotRowsParallel(db, playerUid).then((result) => ({
        ...result,
        durationMs: Date.now() - sqlRequestsStartedAt,
      })),
    ...
    logSnapshotTiming({
      ...auth.timing,
      sql_requests_ms: snapshotPack.durationMs,
      ...
      sqlQueryMode: 'parallel_recent_and_active',
    });
