...
function normalizeFirebase(doc) {
  ...
  return {
    ...
    workerUid: clean(data.workerUid),
    workerRole: clean(data.workerRole),
    workerUsername: clean(data.workerUsername),
...
function normalizeSql(row) {
  return {
    ...
    workerUid: clean(row.worker_uid),
    workerRole: clean(row.worker_role),
    workerUsername: clean(row.worker_username),
...
async function main() {
  ...
  const [firebaseSnap, latestSnap, sqlResult] = await Promise.all([
    ...
    db.collection('rewardCuts').orderBy('createdAt', 'desc').limit(50).get(),
    pool.query('SELECT * FROM public.reward_cuts_cache WHERE deleted_at IS NULL'),
  ]);
...
main().catch((error) => {
  console.error('[COMPARE_REWARD_CUTS_CACHE] fatal', error);
  process.exitCode = 1;
});
