...
function normalize(doc) {
  ...
  return {
    firebase_id: doc.id,
    from_uid: clean(data.fromUid || data.senderUid),
    from_username: clean(data.fromUsername || data.senderUsername),
    to_uid: clean(data.toUid || data.targetUid),
    to_username: clean(data.toUsername || data.targetUsername),
    coadmin_uid: clean(data.coadminUid || data.createdBy),
    amount_coins: numOrNull(data.amountCoins),
    fee_coins: numOrNull(data.feeCoins),
    received_coins: numOrNull(data.receivedCoins || data.recipientCoins),
    fee_percent: numOrNull(data.feePercent),
...
async function existingIds(pool) {
  const result = await pool.query('SELECT firebase_id FROM public.player_coin_rewards_cache WHERE deleted_at IS NULL');
  ...
  await pool.query(
    `
      INSERT INTO public.player_coin_rewards_cache (
        ...
        created_at, raw_firestore_data, source, mirrored_at, deleted_at
      )
      VALUES (
        $1, NULLIF($2, ''), NULLIF($3, ''), NULLIF($4, ''), NULLIF($5, ''),
        NULLIF($6, ''), $7, $8, $9, $10,
        $11::timestamptz, $12::jsonb, 'firebase_backfill', now(), NULL
      )
    ...
    `,
    [
      row.firebase_id, row.from_uid, row.from_username, row.to_uid, row.to_username,
      row.coadmin_uid, row.amount_coins, row.fee_coins, row.received_coins, row.fee_percent,
      row.created_at, JSON.stringify(row.raw_firestore_data),
    ]
  );
}
...
async function main() {
  ...
  let query = db.collection('playerCoinRewards');
  ...
  for (const doc of snapshot.docs) {
    ...
    try {
      ...
      errors += 1;
      console.error('[BACKFILL_PLAYER_COIN_REWARDS_CACHE] failed', { rewardId: doc.id, error });
    }
  }
...
main().catch((error) => {
  console.error('[BACKFILL_PLAYER_COIN_REWARDS_CACHE] fatal', error);
  process.exitCode = 1;
});
