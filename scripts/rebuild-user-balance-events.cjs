...
async function buildEvents(pool) {
  ...
  for (const row of financialRows) {
    ...
    else if (type === 'redeem_refund') add(events, makeEvent({ ...common, event_key: `financialEvents:${id}:${userUid}:cash:redeem_refund_cash_credit`, balance_type: 'cash', delta: amount, event_type: 'redeem_refund_cash_credit', confidence: 'medium', source_fields: { amount } }, profiles));
