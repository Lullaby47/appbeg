...
async function main() {
  ...
  try {
    const [firebaseSnap, sqlResult] = await Promise.all([
      ...
      pool.query(`
        SELECT firebase_id
        FROM public.player_coin_rewards_cache
    ...
    if (!DRY_RUN) {
      for (const firebaseId of missingInFirebase) {
        try {
          await pool.query(
            `
              UPDATE public.player_coin_rewards_cache
              SET deleted_at = now(),
                  mirrored_at = now()
              WHERE firebase_id = $1
          ...
          errors += 1;
          console.error('[RECONCILE_PLAYER_COIN_REWARDS_CACHE_TOMBSTONES] failed', {
            firebaseId,
            error,
          });
        }
...
main().catch((error) => {
  console.error('[RECONCILE_PLAYER_COIN_REWARDS_CACHE_TOMBSTONES] fatal', error);
  process.exitCode = 1;
});
