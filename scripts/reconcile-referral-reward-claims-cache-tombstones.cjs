    ...
    if (!DRY_RUN) {
      for (const firebaseId of missingInFirebase) {
        try {
          await pool.query(
            `
              UPDATE public.referral_reward_claims_cache
              SET deleted_at = now(),
                  mirrored_at = now()
              WHERE firebase_id = $1
          ...
          errors += 1;
          console.error('[RECONCILE_REFERRAL_REWARD_CLAIMS_CACHE_TOMBSTONES] failed', {
            firebaseId,
            error,
          });
        }
...
main().catch((error) => {
  console.error('[RECONCILE_REFERRAL_REWARD_CLAIMS_CACHE_TOMBSTONES] fatal', error);
  process.exitCode = 1;
});
